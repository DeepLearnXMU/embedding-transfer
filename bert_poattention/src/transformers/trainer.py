...
def sample_worker(input_queue, output_queue, src_dict, tgt_dict, tokenizer):
...
#         for sample_idx in range(len(samples)):
        try:
            dict_pad_index = tokenizer._convert_token_to_id_with_added_voc('[PAD]')
            dict_eos_index = tokenizer._convert_token_to_id_with_added_voc('[EOS]')
            need_partition_rate = 0.4
            partition_rate = 0.2
            merge_rate = 0.5
            target_total_gather_indexs = []
            temp_tgt_dict = copy.deepcopy(tgt_dict)
            max_num_target_tokens = 0 
            target_max_gather_index_lens = 1
            ...
            padding_idx = 99999
            for line_idx, line_tokens in enumerate(inputs['input_ids'][:512]):
    ...
    #             print(line_tokens)
                words = [tokenizer._convert_id_to_token(word_idx) for word_idx in [int(idx) for idx in line_tokens] if not (word_idx == dict_pad_index)]
    #             print(words)
    #             exit()
                gather_index_helper = [i for i in range(0, len(words))]
                partition_words = []
                merge_words = []
                new_words = []
                ...
                for idx, word in enumerate(words):
                    ...
                    if word.replace('##', '').__len__() > 6 and random.random() < need_partition_rate and inputs['labels'][line_idx][idx] == -100: # 先确定要拆成多少个token，随机选择位置，用负数的数值指示token个数，用于后续的整理
                        num_tokens = 1
                        init_idx = 0
                ...
                while idx < words.__len__():
                    if '##' not in words[idx] or gather_index_helper[idx] < 0:
                        ...
                        continue
                    now_idx = idx
                    while now_idx < words.__len__() - 2 and '##' in words[now_idx] and inputs['labels'][line_idx][now_idx] == -100 and random.random() < merge_rate and gather_index_helper[now_idx] > 0 and gather_index_helper[now_idx+1] > 0 and inputs['labels'][line_idx][now_idx+1] == -100:
                        now_idx += 1
                    if now_idx == idx:
                        idx += 1
                        continue
                    # print(gather_index_helper)
                    gather_index_helper.append(len(gather_index_helper))
                ...
                for idx in range(len(words)):
                    ...
                    if helper >= 0 and helper < sentence_len: # normal
                        if int(line_tokens[idx]) != dict_pad_index:
                        ...
                        new_word_index = temp_tgt_dict.add_symbol(new_word, overwrite=True)
                        # print(new_word)
    ...
    #             # bos
    #             for g_idx in range(len(gather_index)):
    #                 gather_index[g_idx] += 1
    #             gather_index = [0] + gather_index
                target_total_gather_indexs.append(gather_index)

                if len(gather_index) > target_max_gather_index_lens:
                    target_max_gather_index_lens = len(gather_index)
                ...
                # print(new_words.__len__())
                # print(target_total_gather_indexs)
                for new_word in new_words:
                    chlds, chlds_idx = find_chld(new_word, tgt_dict, tokenizer)
...
#                     position = [i for i in range(len(chlds_idx))] + [max_sub_position] * len(prts_idx)
    ...
    #         print(target_total_gather_indexs)
    #         print(target_total_chld_prt_tokens)
    #         exit()

            # Add paddings
            token_padding_index = dict_pad_index
            ...
            for gather_index in target_total_gather_indexs:
                gather_index += ([padding_idx] * (target_max_gather_index_lens - len(gather_index)))

            for idx in range(len(target_total_new_target_tokens)):
                # print(target_total_new_target_tokens[-1])
                temp = target_total_new_target_tokens[idx] + [dict_pad_index] * (max_num_target_tokens - len(target_total_new_target_tokens[idx]))
                # print(temp1)
...
#             print(t_total_chld_prt_tokens)
            ...
            # print(target_total_gather_indexs)
            target_total_gather_indexs = torch.LongTensor(target_total_gather_indexs)
            # print(new_input_ids[-1])
            # print(target_total_new_target_tokens)
            # print(new_input_ids.__len__())
            # print([len(item) for item in new_input_ids])
            new_input_ids = torch.LongTensor(new_input_ids)

            # samples[sample_idx]['net_input']['gather_index'] = total_gather_indexs
            inputs['chld_prt_tokens'] = target_total_chld_prt_tokens
#             print(inputs['chld_prt_tokens'])
#             print(inputs['chld_prt_tokens'].size())
            inputs['types'] = target_total_types
            inputs['positions'] = target_total_positions
            inputs['gather_index'] = target_total_gather_indexs

            inputs['old_input_ids'] = inputs['input_ids']
            inputs['input_ids'] = new_input_ids
            add_zeros = -100 * torch.ones(new_input_ids.size(0), 512).long()
            temp_labels = torch.cat([inputs['labels'], add_zeros], dim=1)
    #         print(target_total_gather_indexs)
    #         print(target_total_gather_indexs-((padding_idx - temp_labels.size(1) + 1) * (target_total_gather_indexs==padding_idx).int()))
            new_labels = torch.gather(temp_labels, 1, target_total_gather_indexs-((padding_idx - temp_labels.size(1) + 1) * (target_total_gather_indexs==padding_idx).int()))
    #         print(target_total_gather_indexs.size())
    #         print(new_labels.size())
    #         print(new_input_ids.size())
    #         exit()
            for line_idx in range(inputs['input_ids'].size(0)):
                for word_idx in range(inputs['input_ids'].size(1)):
                    if int(inputs['input_ids'][line_idx][word_idx]) > src_dict.__len__() and random.random() > 0.5:
                        new_labels[line_idx][word_idx] = inputs['input_ids'][line_idx][word_idx]
                        inputs['input_ids'][line_idx][word_idx] = src_dict.index('[MASK]')
                        
            inputs['labels'] = new_labels
            new_attention_mask = (new_input_ids != dict_pad_index).long()
            inputs['attention_mask'] = new_attention_mask
            new_token_type_ids = torch.zeros(new_input_ids.size(0), new_input_ids.size(1)).long()
            inputs['token_type_ids'] = new_token_type_ids

            for key in inputs.keys():
                if key not in ['chld_prt_tokens', 'types', 'positions']:
                    inputs[key] = inputs[key][:3]

            # print(samples)
            # exit()
            output_queue.put(inputs)
        except:
...
#                 if 'embed_generator' not in n:
#                     p.requires_grad = False
            optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in self.model.named_parameters() if not any(nd in n for nd in no_decay) and 'embedding_generator' in n and p.requires_grad],
                    "weight_decay": self.args.weight_decay,
                },
                {
                    "params": [p for n, p in self.model.named_parameters() if any(nd in n for nd in no_decay) and 'embedding_generator' in n and p.requires_grad],
                    "weight_decay": 0.0,
                },
            ]
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        processes = []
        num_workers = 10
        src_dict = self.tokenizer.get_vocab()
        rewrite_dict = Dictionary()
        rewrite_dict.load_from_dict(src_dict)
#         print(rewrite_dict)
#         print(src_dict['[PAD]'])
#         print(rewrite_dict.index('[PAD]'))
#         print(rewrite_dict[0])
#         exit()
        for i in range(num_workers):
            print('Start processing')
            process = Process(target=sample_worker, args=(input_queue, output_queue, rewrite_dict, rewrite_dict, self.tokenizer))
            process.start()
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, now_inputs in enumerate(epoch_iterator):
#                 print(self.tokenizer.get_vocab())
#                 print(now_inputs)
# #                 inputs['test'] = torch.ones([2, 4])
#                 print(self.tokenizer.decode(inputs['input_ids'][0]))
#                 print(self.tokenizer.get_vocab()['Hello'])
#                 exit()
                if step == 0:
                    for j in range(len(processes)):
                        input_queue.put(now_inputs)
#                     print(input_queue.qsize())
                while(output_queue.empty()):
                    time.sleep(0.5)
#                     print(input_queue.qsize())
                    pass
                inputs = output_queue.get()
                input_queue.put(now_inputs)
                if inputs is None:
                    print('continue')
                    continue
