...
class Trainer(object):
    ...
    def _build_optimizer(self):
        # print([p[0] for p in list(filter(
        ...
        # )
        params = list(
            [p[1] for p in list(filter(
                lambda p: p[1].requires_grad and 'embed_generator' in p[0],
                chain(self.model.named_parameters(), self.criterion.named_parameters()),
            ))]
        )
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            now_model_state = self.get_model().state_dict()
            # print(now_model_state.keys())
            for name in now_model_state.keys():
                if name not in model_state:
                    print(name)
                    model_state[name] = now_model_state[name]

            # load model parameters
            try:
                self.get_model().load_state_dict(
                    model_state, strict=True, args=self.args
                )
